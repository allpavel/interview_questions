...
Репозиторий представляет собой коллецию вопросов с различных собеседований на вакансию Junior/Middle Front-end Developer.
...
HTML (от англ. _HyperText Markup Language_ — «язык гипертекстовой разметки») - это код, который используется для структурирования веб-страницы и её контента. HTML предоставляет средства для создания заголовков, абзацев, списков, ссылок, цитат и других элементов. Элементы HTML выделяются тегами, записанными с использованием угловых скобок. Например, `<img />`, `<input />` или `<p>`. Регистр, в котором набрано имя тега, значения не имеет.
Элементы могут быть пустыми, т.е. не содержать никакого теста и других данных (например, `<hr>`). Помимо этого, открывающие теги могут содержать атрибуты, которые определяют какие-либо их свойства. Например, атрибут "href" тега `<a>`: `<a href="#top">Вернуться наверх</a>`
...
1. Элемент верхнего уровня - корневой элемент на веб-странице (например, html);
2. Публичность - определяет является ли объект публичным объектом (PUBLIC) или системным ресурсом (SYSTEM), таким как локальный файл или URL. Публичными идентификаторами могут быть HTML/XHTML DTD, тогда как XML DTD могут быть как публичными, так и системными.
3. Регистрация - указывает на регистрацию организации в ISO. Знак "+" означает наличие регистрации, знак "-" её отсутствие. На изображении выше W3C не имеет регистрации в ISO, следовательно, используется знак "-".
4. Организация - уникальный ID организации или разработчика ("OwnerID"), отвечающего за DTD, на который ссылается DOCTYPE.
5. Тип - определяет тип объекта (страницы), на который ссылаются ("Public Text Class"). Например, в случае HTML/XHTML DTD тип документа - DTD (Document Type Definition).
6. Имя - уникальное имя документа, на который ссылаются ("Public Text Description").
7. Язык - система кодирования естественного языка, используемая при создании документа, на который даётся ссылка ("Public Text Language"). Записывается согласно ISO 639 (две заглавные буквы). Например, на изображении выше это "EN".
8. URL - адрес страницы, на которой находится описание для указанного типа документа.
...
&nbsp;
  ...
  HTML-сущности (мнемоники) — это части текста ("строки"), которые начинаются с символа амперсанда (&) и заканчиваются точкой с запятой (;). Некоторые специальные символы, если они появляются в документе HTML, могут вызвать недопонимание в синтаксическом анализаторе исходного кода HTML, например, символ "меньше" ( < ), при встрече с данным символом анализатор HTML может ошибочно принять этот символ за тег. Чтобы избежать данное недопонимание, можно использовать специальный объект (entity) &lt; чтобы заменить символ ( < ).
  Так же html-сущности могут быть полезны, когда требуется использовать символ, которого нет на клавиатуре. При этом нужно помнить о том, что такие спец.символы теряют свой форматный смысл.

Прочитать:

1. [Мнемоники в HTML](https://ru.wikipedia.org/wiki/%D0%9C%D0%BD%D0%B5%D0%BC%D0%BE%D0%BD%D0%B8%D0%BA%D0%B8_%D0%B2_HTML)
2. [Таблицы ссылок-мнемоник](https://ru.wikipedia.org/wiki/%D0%9C%D0%BD%D0%B5%D0%BC%D0%BE%D0%BD%D0%B8%D0%BA%D0%B8_%D0%B2_HTML#%D0%A2%D0%B0%D0%B1%D0%BB%D0%B8%D1%86%D1%8B_%D1%81%D1%81%D1%8B%D0%BB%D0%BE%D0%BA-%D0%BC%D0%BD%D0%B5%D0%BC%D0%BE%D0%BD%D0%B8%D0%BA)
...
data-\* атрибуты позволяют хранить дополнительную информацию в стандартных элементах HTML без визуального представления. Как правило в таких атрибутах хранят значения, которые потом используют в CSS или JavaScript. Например: `<div data-id="404">This is an example.</div>`
...
&nbsp;
  ...
  Например, к глобальным атрибутам относятся:
  - style - содержит объявления CSS стилей, которые могут применяться к элементу;
  - class - задает один или несколько классов для элемента для того, чтобы затем обратиться к необходимому элементу или группе элементов при помощи CSS или JavaScript;
  - id - определяет уникальный идентификатор, который должен быть уникальным для всего документа. Используется для идентификации элемента. Элементы могут иметь только один id;
  - hidden - атрибут, определяющий будет ли браузер рендерить элемент;
  - autofocus - определяет, что элементу будет присвоен фокус при загрузке страницы. Этот атрибут может иметь только один элемент на странице;
  - role - определяет семантическое значение контента, которое позволяет скринридерам и другим инструментам взаимодествовать с различными объектами так, как этого ожидают пользователи. 
  ...
  3. [ARIA-атрибуты](https://doka.guide/a11y/aria-attrs/)
  ...
  1. Поисковые системы будут рассматривать содержимое такой разметки как важные ключевые слова, влияющие на рейтинг страницы в результатах поиска.
  ...
  3. Семантические теги повышают читаемость кода.
  4. Указывает разработчику на тип данных, которые будут размещены.
  5. Семантическая разметка отражает правильное именование элементов.
  ...
  Элемент div оборачивает другие элементы и семантически ничего не значит, а отвечает лишь за представление. При этом при помощи ARIA атрибутов можно имитировать доступность или другие HTML теги.
  ...
  Семантическая вёрстка — подход к разметке, который опирается не на содержание сайта, а на смысловое предназначение каждого блока и логическую структуру документа. Семантическая верстка является основой для веб-доступности, т.к. вспомогательные (ассистивные) технологии (технологие, которые помогают пользователям с ограниченными возможностями просматривать веб-страницы) зависят от семантической структуры для передачи информации пользователям.
...
3. [Semantic HTML: the foundation of web accessibility](https://uxdesign.cc/semantic-html-the-foundation-of-web-accessibility-e5bbecad7c17)
  ...
  HTML5 предлагает набор секционных (структурных) элементов, используя которые вы добавляете смысловую или семантическую нагрузку своим страницам, тем самым позволяя компьютерным программам лучше понимать их содержание и значение, не опираясь на представление. Так же семантическая верстка позволяет девелоперам лучше понимать архитектуру кода, без знания контента, т.к. семантические теги обеспечивают структуру и значение элементов. К секционным элементам относятся: ```header```, ```footer```, ```main```, ```aside```, ```article```, ```section```, ```nav```, ```h1-h6```.
  ...
  3. [Headings and sections](https://web.dev/learn/html/headings-and-sections/)
...
React - это библиотека для создания пользовательских интерфейсов. Реакт-приложения состоят из компонентов - частей пользовательского интерфейса, которые имеют собственную логику и представление. Компоненты Реакта представляют собой функции, которые возвращают разметку.
...
3. [What Is React.js? A Look at the Popular JavaScript Library](https://kinsta.com/knowledgebase/what-is-react-js/)
...
SPA (Single Page Application) - это веб-приложение, в котором содержимое страницы, маршрутизация и логика, необходимые для отображния контента, обрабатываются с помощью JavaScript в браузере. При взаимодействии пользователя с приложением не происходит полной перезагрузки страницы, а только динамечески изменяются конкретные элементы в рамках одного HTML документа. При этом достигается высокая скорость и отзывчивость интерфейсов, что значительно улучшает UX, но, с другой стороны, страдает SEO, увеличивается нагрузка на браузер, т.к. ему приходится обрабатывать всё больше и больше JavaScript'а, что может привести к обратному эффекту в виде замедления первоначальной загрузки сайта. При этом пользователь какое-то время может видеть только лишь какой-нибудь лоадер, либо, в худшем случае, просто белую страницу.
